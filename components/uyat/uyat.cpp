...
void Uyat::setup() {
  this->set_interval("heartbeat", 15000, [this] { this->send_empty_command_(UyatCommandType::HEARTBEAT); });
...
void Uyat::dump_config() {
  ...
  if (this->init_state_ != UyatInitState::INIT_DONE) {
    ...
    ESP_LOGCONFIG(TAG, "  If no further output is received, confirm that this is a supported Uyat device.");
    return;
  }
...
bool Uyat::validate_message_() {
  ...
  if (rx_checksum != calc_checksum) {
    ESP_LOGW(TAG, "Uyat Received invalid message checksum %02X!=%02X", rx_checksum, calc_checksum);
    return false;
  }

  // valid message
  const uint8_t *message_data = data + 6;
  ESP_LOGV(TAG, "Received Uyat: CMD=0x%02X VERSION=%u DATA=[%s] INIT_STATE=%u", command, version,
...
void Uyat::handle_command_(uint8_t command, uint8_t version, const uint8_t *buffer, size_t len) {
  ...
  switch (command_type) {
      ...
      if (this->init_state_ == UyatInitState::INIT_CONF) {
        ...
        if (this->status_pin_reported_ != -1) {
          ...
          if (is_pin_equals) {
            ...
            ESP_LOGW(TAG, "Supplied status_pin does not equals the reported pin %i. UyatMcu will work in limited mode.",
    ...
    case UyatCommandType::DATAPOINT_REPORT_SYNC:
      ...
      if (command_type == UyatCommandType::DATAPOINT_REPORT_SYNC) {
        this->send_command_(
            UyatCommand{.cmd = UyatCommandType::DATAPOINT_REPORT_ACK, .payload = std::vector<uint8_t>{0x01}});
      }
      break;
    case UyatCommandType::DATAPOINT_QUERY:
      break;
    case UyatCommandType::WIFI_TEST:
      this->send_command_(UyatCommand{.cmd = UyatCommandType::WIFI_TEST, .payload = std::vector<uint8_t>{0x00, 0x00}});
      break;
    case UyatCommandType::WIFI_RSSI:
      this->send_command_(
          UyatCommand{.cmd = UyatCommandType::WIFI_RSSI, .payload = std::vector<uint8_t>{get_wifi_rssi_()}});
      break;
    ...
    case UyatCommandType::VACUUM_MAP_UPLOAD:
      this->send_command_(
          UyatCommand{.cmd = UyatCommandType::VACUUM_MAP_UPLOAD, .payload = std::vector<uint8_t>{0x01}});
    ...
    case UyatCommandType::GET_NETWORK_STATUS: {
      ...
      this->send_command_(
          UyatCommand{.cmd = UyatCommandType::GET_NETWORK_STATUS, .payload = std::vector<uint8_t>{wifi_status}});
    ...
    case UyatCommandType::EXTENDED_SERVICES: {
      ...
      switch ((UyatExtendedServicesCommandType) subcommand) {
        case UyatExtendedServicesCommandType::RESET_NOTIFICATION: {
          this->send_command_(
              UyatCommand{.cmd = UyatCommandType::EXTENDED_SERVICES,
                          .payload = std::vector<uint8_t>{
                              static_cast<uint8_t>(UyatExtendedServicesCommandType::RESET_NOTIFICATION), 0x00}});
...
void Uyat::send_raw_command_(UyatCommand command) {
  ...
  ESP_LOGV(TAG, "Sending Uyat: CMD=0x%02X VERSION=%u DATA=[%s] INIT_STATE=%u", static_cast<uint8_t>(command.cmd),
...
void Uyat::send_wifi_status_() {
  ...
  this->send_command_(UyatCommand{.cmd = UyatCommandType::WIFI_STATE, .payload = std::vector<uint8_t>{status}});
}
...
void Uyat::send_local_time_() {
  ...
  this->send_command_(UyatCommand{.cmd = UyatCommandType::LOCAL_TIME_QUERY, .payload = payload});
}
#endif

void Uyat::set_raw_datapoint_value(uint8_t datapoint_id, const std::vector<uint8_t> &value) {
...
void Uyat::set_boolean_datapoint_value(uint8_t datapoint_id, bool value) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::BOOLEAN, value, 1, false);
}

void Uyat::set_integer_datapoint_value(uint8_t datapoint_id, uint32_t value) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::INTEGER, value, 4, false);
}

void Uyat::set_string_datapoint_value(uint8_t datapoint_id, const std::string &value) {
...
void Uyat::set_enum_datapoint_value(uint8_t datapoint_id, uint8_t value) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::ENUM, value, 1, false);
}

void Uyat::set_bitmask_datapoint_value(uint8_t datapoint_id, uint32_t value, uint8_t length) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::BITMASK, value, length, false);
}

void Uyat::force_set_raw_datapoint_value(uint8_t datapoint_id, const std::vector<uint8_t> &value) {
...
void Uyat::force_set_boolean_datapoint_value(uint8_t datapoint_id, bool value) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::BOOLEAN, value, 1, true);
}

void Uyat::force_set_integer_datapoint_value(uint8_t datapoint_id, uint32_t value) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::INTEGER, value, 4, true);
}

void Uyat::force_set_string_datapoint_value(uint8_t datapoint_id, const std::string &value) {
...
void Uyat::force_set_enum_datapoint_value(uint8_t datapoint_id, uint8_t value) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::ENUM, value, 1, true);
}

void Uyat::force_set_bitmask_datapoint_value(uint8_t datapoint_id, uint32_t value, uint8_t length) {
  this->set_numeric_datapoint_value_(datapoint_id, UyatDatapointType::BITMASK, value, length, true);
}
...
void Uyat::set_numeric_datapoint_value_(uint8_t datapoint_id, UyatDatapointType datapoint_type, const uint32_t value,
...
void Uyat::set_raw_datapoint_value_(uint8_t datapoint_id, const std::vector<uint8_t> &value, bool forced) {
...
void Uyat::set_string_datapoint_value_(uint8_t datapoint_id, const std::string &value, bool forced) {
...
void Uyat::send_datapoint_command_(uint8_t datapoint_id, UyatDatapointType datapoint_type, std::vector<uint8_t> data) {
  ...
  this->send_command_(UyatCommand{.cmd = UyatCommandType::DATAPOINT_DELIVER, .payload = buffer});
}

void Uyat::register_listener(uint8_t datapoint_id, const std::function<void(UyatDatapoint)> &func) {
